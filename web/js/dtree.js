	...
	if (this.config.useIcons) {
		...
		if (this.config.useRadio && node.id != 0) {
			str += '<input type="radio" name="chk" id="r' + this.obj + nodeId
					+ '" onclick="showParentMenuNameByClickRadio(' + nodeId + ')" value="' + node.id + '"/>';
		}
		...
		if (this.config.useCheckbox == true && node.id != 0) {
			str += '<input type="checkbox" id="c' + this.obj + nodeId
					+ '" onclick="javascript:' + this.obj + '.selectCheckbox(' + nodeId + ')"/>';
		}
	}
	...
	do{
		...
		for(n=0;n<len;n++){
			if(this.aNodes[n].id==pid){
				if(n != 0){     //因为id="c"+this.obj+n 是根节点,前面没有复选框,所以不需要下面的步骤
